...
categories = legendfamiliesconfig.get_categories()
...
with revit.Transaction("List Symbols"):
    for cat in sorted(ordered_symbols):
        ...
        # cat_text_note = DB.TextNote.Create(revit.doc, legend.Id, cat_label_position, cat, get_any_text_type_id())
        ...
        for fam in ordered_symbols[cat]:
            ...
            f_text_note = DB.TextNote.Create(revit.doc, legend.Id, label_position, fam, get_any_text_type_id())
            ...
            for fam_type in ordered_symbols[cat][fam]:
                ...
                if not sym.IsActive:
                    sym.Activate()
                copy_component_id = DB.ElementTransformUtils.CopyElement(revit.doc, source_element.Id, position)[0]
                new_component = revit.doc.GetElement(copy_component_id)
                # CHANGE TYPE HERE
                new_component.get_Parameter(DB.BuiltInParameter.LEGEND_COMPONENT).Set(sym.Id)
                new_component.get_Parameter(DB.BuiltInParameter.LEGEND_COMPONENT_VIEW).Set(-7)
                # new_tag_symbol = revit.doc.Create.NewFamilyInstance(position, sym, legend)
                ...
                # text_note = DB.TextNote.Create(revit.doc, legend.Id, label_position, fam_type, get_any_text_type_id())
                # position = DB.XYZ(pt.X, (position.Y-(bb_h*scale)-offset), 0)
                position = DB.XYZ(pt.X, (position.Y-(bb_h*scale)*4-offset), 0)
            position = DB.XYZ(pt.X, (position.Y-offset), 0)
